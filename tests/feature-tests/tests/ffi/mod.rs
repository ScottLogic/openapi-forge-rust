...
pub fn drop_api_client_if_exists(w: &mut ForgeWorld) -> Result<()> {
    unsafe {
        ...
        if let Some(api_client) = api_client {
            if let Some(library) = &w.library {
                let func: Symbol<extern "C" fn(Box<ApiClient>)> = library.get(
                    b"c_api_client_drop"
                )?;
                func(api_client);
            } else {
...
pub fn serialize_returned_variable<T>(
    ...
    unsafe {
        ...
        let c_method_bytes = c_method.as_bytes();
        if let Some(library) = &w.library {
            let func: Symbol<extern "C" fn(Box<ForgeResponse<T>>) -> FFISafeTuple<T>> = library.get(
                c_method_bytes
            )?;
...
pub fn run_method_no_params<T>(
    ...
    unsafe {
        ...
        let c_method_bytes = c_method.as_bytes();
        if let Some(library) = &w.library {
            let func: Symbol<extern "C" fn(Box<ApiClient>) -> Box<ForgeResponse<T>>> = library.get(
                c_method_bytes
            )?;
            let api_client = w.api_client.take();
            if let Some(api_client) = api_client {
